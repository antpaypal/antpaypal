...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 4)
        throw runtime_error(
            "importprivkey <antpaypalprivkey> [label] [rescan=true] [timestamp=1]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <antpaypaladdress/public_key_hex>\n"
            "Reveals the private key corresponding to <antpaypaladdress>.");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Antpaypal %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
