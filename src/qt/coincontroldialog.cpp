...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog, AntpaypalBridge *bridge)
{
    if (!model) return;
    ...
    } else
    {
        ...
        int nDisplayUnit = BitcoinUnits::APP;
        ...
        l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01APP
