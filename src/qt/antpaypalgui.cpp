// Copyright (c) 2014 The Antpaypal developers
...
AntpaypalGUI::AntpaypalGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new AntpaypalBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("蚂蚁资产") + " - " + tr("客户端"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/antpaypal"));
    setWindowIcon(QIcon(":icons/antpaypal"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/antpaypal/index.html");
#else
    QFile html("/opt/antpaypal/index.html");
#endif
...
void AntpaypalGUI::pageLoaded(bool ok)
{
...
void AntpaypalGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void AntpaypalGUI::urlClicked(const QUrl & link)
{
...
void AntpaypalGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/antpaypal"), tr("&About Antpaypal"), this);
    aboutAction->setToolTip(tr("Show information about Antpaypal"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Antpaypal"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/antpaypal"), tr("&Show / Hide"), this);
...
void AntpaypalGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void AntpaypalGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("蚂蚁资产") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/antpaypal_testnet"));
            setWindowIcon(QIcon(":icons/antpaypal_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/antpaypal_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("蚂蚁资产 客户端") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/antpaypal_testnet"));
...
void AntpaypalGUI::setWalletModel(WalletModel *walletModel)
{
...
void AntpaypalGUI::setMessageModel(MessageModel *messageModel)
{
...
void AntpaypalGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("蚂蚁资产 客户端"));
    trayIcon->setIcon(QIcon(":/icons/antpaypal"));
...
#ifndef Q_OS_MAC
void AntpaypalGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AntpaypalGUI::aboutClicked()
{
    AboutDialog dlg;
...
void AntpaypalGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to Antpaypal network", "", count));
}

void AntpaypalGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void AntpaypalGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void AntpaypalGUI::changeEvent(QEvent *e)
{
...
void AntpaypalGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void AntpaypalGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::APP, nFeeRequired));
...
void AntpaypalGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void AntpaypalGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void AntpaypalGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void AntpaypalGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AntpaypalGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void AntpaypalGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Antpaypal address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void AntpaypalGUI::handleURI(QString strURI)
{
...
void AntpaypalGUI::setEncryptionStatus(int status)
{
...
void AntpaypalGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void AntpaypalGUI::backupWallet()
{
...
void AntpaypalGUI::changePassphrase()
{
...
void AntpaypalGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void AntpaypalGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void AntpaypalGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AntpaypalGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void AntpaypalGUI::updateStakingIcon()
{
...
void AntpaypalGUI::detectShutdown()
{
