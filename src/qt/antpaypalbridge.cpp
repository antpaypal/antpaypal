#include "antpaypalbridge.h"

#include "antpaypalgui.h"
#include "guiutil.h"
...
#include "antpaypalbridge.moc"

AntpaypalBridge::AntpaypalBridge(AntpaypalGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
AntpaypalBridge::~AntpaypalBridge()
{
...
void AntpaypalBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void AntpaypalBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void AntpaypalBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void AntpaypalBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void AntpaypalBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void AntpaypalBridge::populateOptions()
{
...
// Transactions
bool AntpaypalBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void AntpaypalBridge::clearRecipients()
{
    recipients.clear();
}

bool AntpaypalBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 APP, 1 Antpaypal
        switch(rcp.txnTypeInd)
        {
            case TXT_APP_TO_APP:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APP, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_APP_TO_ANON:
                formatted.append(tr("<b>%1</b> to ANTPAYPAL %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APP, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> ANTPAYPAL, ring size %2 to ANTPAYPAL %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APP, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_APP:
                formatted.append(tr("<b>%1</b> ANTPAYPAL, ring size %2 to APP %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APP, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APP, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsAntpaypalBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your antpaypal balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::APP, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void AntpaypalBridge::openCoinControl()
{
...
void AntpaypalBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void AntpaypalBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap AntpaypalBridge::listAnonOutputs()
{
...
void AntpaypalBridge::populateTransactionTable()
{
...
void AntpaypalBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void AntpaypalBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString AntpaypalBridge::transactionDetails(QString txid)
{
...
// Addresses
void AntpaypalBridge::populateAddressTable()
{
...
void AntpaypalBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void AntpaypalBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString AntpaypalBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString AntpaypalBridge::lastAddressError()
{
    QString sError;
...
QString AntpaypalBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void AntpaypalBridge::updateAddressLabel(QString address, QString label)
{
...
bool AntpaypalBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool AntpaypalBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void AntpaypalBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void AntpaypalBridge::appendMessage(int row)
{
...
void AntpaypalBridge::populateMessageTable()
{
...
void AntpaypalBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool AntpaypalBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool AntpaypalBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString AntpaypalBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString AntpaypalBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool AntpaypalBridge::setPubKey(QString address, QString pubkey)
{
...
bool AntpaypalBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString AntpaypalBridge::createGroupChat(QString label)
{
...
QString AntpaypalBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList AntpaypalBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void AntpaypalBridge::connectSignals()
{
...
QString AntpaypalBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap AntpaypalBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap AntpaypalBridge::listLatestBlocks()
{
...
QVariantMap AntpaypalBridge::findBlock(QString searchID)
{
...
QVariantMap AntpaypalBridge::blockDetails(QString blkHash)
{
...
QVariantMap AntpaypalBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap AntpaypalBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap AntpaypalBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap AntpaypalBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap AntpaypalBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap AntpaypalBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap AntpaypalBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap AntpaypalBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap AntpaypalBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Antpaypal prefix.");
            return result;
        }
...
QVariantMap AntpaypalBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap AntpaypalBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap AntpaypalBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
