...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return AntpaypalGUI::tr("Received with");
    case RecvFromOther:
        return AntpaypalGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return AntpaypalGUI::tr("Sent to");
    case SendToSelf:
        return AntpaypalGUI::tr("Payment to yourself");
    case Generated:
        return AntpaypalGUI::tr("Mined");
    case RecvAntpaypal:
        return AntpaypalGUI::tr("Received antpaypal");
    case SendAntpaypal:
        return AntpaypalGUI::tr("Sent antpaypal");
    case Other:
        return AntpaypalGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvAntpaypal:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendAntpaypal:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredAPP, nCredAntpaypal;
    wtx.GetCredit(nCredAPP, nCredAntpaypal, true);
    int64_t nCredit = nCredAPP + nCredAntpaypal;
    ...
    {
        if (nNet > 0 && nCredAntpaypal > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvAntpaypal, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendAntpaypal, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvAntpaypal;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv antpaypal
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendAntpaypal;
                    sub.address = CBitcoinAddress(ckidD).ToString();
