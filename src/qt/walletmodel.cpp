...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedAntpaypalBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getAntpaypalBalance() const
{
    return wallet->GetAntpaypalBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newAntpaypalBal = getAntpaypalBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedAntpaypalBal != newAntpaypalBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newAntpaypalBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("Antpaypal is not connected!"));
    ...
    {
        nBalance = wallet->GetAntpaypalBalance();
        if ((nTotalOut + MIN_TX_FEE_ANON) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsAntpaypalBalance, MIN_TX_FEE_ANON);
    };

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_APP_TO_APP
                || rcp.txnTypeInd == TXT_ANON_TO_APP)
            {
                // -- out app
                std::string sError;
