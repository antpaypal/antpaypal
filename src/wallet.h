...
public:
    ...
    int64_t GetAntpaypalBalance() const;
    ...
    bool SendAppToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendAnonToApp(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    int64_t GetAntpaypalDebit(const CTxIn& txin) const;
    int64_t GetAntpaypalCredit(const CTxOut& txout) const;
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetAntpaypalCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nAPP, int64_t& nAntpaypal) const
    {
        nAPP = 0;
        nAntpaypal = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nAntpaypal += GetAntpaypalCredit(txout);
            } else
                nAPP += GetCredit(txout);
            if (!MoneyRange(nAPP)
                || !MoneyRange(nAntpaypal))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailableAntpaypalCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableAntpaypalCreditCached;
    
    mutable int64_t nCredAPPCached;
    mutable int64_t nCredAntpaypalCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableAntpaypalCreditCached = false;
        
        nCredAPPCached = 0;
        nCredAntpaypalCached = 0;
        ...
        nAvailableAntpaypalCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredAPP, int64_t& nCredAntpaypal, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredAPP = nCredAntpaypal = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredAPPCached = 0;
            nCredAntpaypalCached = 0;
            pwallet->GetCredit(*this, nCredAPPCached, nCredAntpaypalCached);
            fCreditSplitCached = true;
        };
        
        nCredAPP = nCredAPPCached;
        nCredAntpaypal = nCredAntpaypalCached;
        return true;
    }
    ...
    int64_t GetAvailableAntpaypalCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableAntpaypalCreditCached)
            return nAvailableAntpaypalCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableAntpaypalCredit() : value out of range");
            };
        };
        
        nAvailableAntpaypalCreditCached = nCredit;
        fAvailableAntpaypalCreditCached = true;
        return nCredit;
    };
